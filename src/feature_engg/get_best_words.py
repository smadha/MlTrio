...
if __name__ == '__main__':
    pair_features = [("distinguish_word.p",get_word_pairs)]
    ...
    for file_name, get_pairs in pair_features:
        ...
        for pair in list_all_pairs:
            ...
            if abs(score)  > abs_threshold and (freq_0 > 10 or freq_1 > 10):
                best_user_words.add(pair[0])
                best_ques_words.add(pair[1])
            
        
        print "len(best_user_words)", len(best_user_words)
        print "len(best_ques_words)", len(best_ques_words)
            
        
        print "pairs generated for", len(pair_score)
        pickle.dump( best_user_words, open("./feature/best_user_words.p", "wb"), protocol=2 )
        pickle.dump( best_ques_words, open("./feature/best_ques_words.p", "wb"), protocol=2 )
